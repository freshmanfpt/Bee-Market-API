...
router.get("/", async (req, res) => {
  ...
  let total = 0;
  let profit = 0;
    const productSold = await Product.find({ status : "Đã bán"});
    const productSoldCount = await Product.find({ status : "Đã bán"}).count({});
...
router.get("/sales/:category", async (req, res) => {
    ...
    let profit = 0;
      const { category } = req.params;
      const productList = await Product.find({ category: category , status : "Đã bán"});
      const productCount = await Product.find({ category: category ,  status : "Đã bán"}).count({});
     productList.map((x)=>{
         profit = profit + parseInt(x.priceIn); 
         total = total +  parseInt(x.priceOut); 
     })
